...
public int linearSearch(int catNumToFind)
{
  for (int i = 0; i < store.length; i++)
    if (store[i].myCatNum == catNumToFind)
      return store[i].myInventory;
  return -1;
}
public int recursiveLinearSearch(int catNumToFind, int startIndex)
{
  if (startIndex >= store.length)
    return -1;
  if (store[startIndex].myCatNum == catNumToFind)
    return store[startIndex].myInventory;
...
public int binarySearch(int catNumToFind)
{
  ...
  while (low <= high) {
    ...
    if (store[guess].myCatNum == catNumToFind)
      return guess;
    else if (store[guess].myCatNum < catNumToFind)
      low = guess + 1;
    else 
       high = guess - 1;
  }
